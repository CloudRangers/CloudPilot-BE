...
public class PrometheusController {
    ...
    private final VmResourceMetricsService vmResourceMetricsService;
    ...
    /**
     * vCenter VM CPU/메모리 사용률 시계열 (Recharts용)
     *
     * 예:
     *   GET /monitor/prometheus/vms/{vmName}/resources?rangeMinutes=60&stepSeconds=60
     */
    @GetMapping("/vms/{vmName:.+}/resources")
    public ApiResponse<VmResourceChartResponse> getVmResourceSeries(
            @PathVariable("vmName") String vmName,
            @RequestParam(value = "rangeMinutes", required = false) Integer rangeMinutes,
            @RequestParam(value = "stepSeconds", required = false) Integer stepSeconds
    ) {
        log.info("[PrometheusController] VM resource series. vmName={}, rangeMinutes={}, stepSeconds={}",
                vmName, rangeMinutes, stepSeconds);

        VmResourceChartResponse data =
                vmResourceMetricsService.getVmResourceSeries(vmName, rangeMinutes, stepSeconds);
